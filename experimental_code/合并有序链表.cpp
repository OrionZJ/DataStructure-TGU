...
Sqlist MergeData(Sqlist &a, Sqlist &b, int sort);
...
int main(){
	...
	Sqlist seqListA;
	InitList(seqListA);
	InsertMutiData(seqListA,i,e);
	cout << "Second SeqList:\n";
	Sqlist seqListB;
	InitList(seqListB);
	InsertMutiData(seqListB,i,e);
	Sqlist merged = MergeData(seqListA, seqListB, 1);
	for (int i = 0; i < merged.length; i++) {
		cout << merged.data[i] << endl;
	}
	return 0;	
}
...
Sqlist MergeData(Sqlist &a, Sqlist &b, int sort) {
    Sqlist mergedList ;
	int Listlength = a.length + b.length;
	mergedList.length = 0;
	mergedList.listsize = a.listsize + b.listsize;
    mergedList.data = (dataType*)malloc(mergedList.listsize*sizeof(dataType));
    int pointA = 0, pointB = 0;
    for (int i = 0; i < Listlength; i++) {
        if (a.data[pointA]*sort <= b.data[pointB]*sort && pointA < a.length && pointB < b.length) {
            InsertData(mergedList, i + 1, a.data[pointA]);
            pointA++;
        } 
		else if (a.data[pointA]*sort > b.data[pointB]*sort && pointA < a.length && pointB < b.length) {
            InsertData(mergedList, i + 1, b.data[pointB]);
            pointB++;
        }
		else if (pointA >= a.length) {
			InsertData(mergedList, i + 1, b.data[pointA]);
            pointB++;
		}
